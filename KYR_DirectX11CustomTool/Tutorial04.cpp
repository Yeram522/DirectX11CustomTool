...
struct PERFRAME { D3DXCOLOR Color; FLOAT X, Y, Z; };// constant buffer 넘길때 자료구조. shader이랑 짝꿍
...
void InitPipeline()
{
    ...
    // constant Buffer의 사용부

    D3D11_BUFFER_DESC bd;
    ZeroMemory(&bd, sizeof(bd));

    bd.Usage = D3D11_USAGE_DEFAULT;
    bd.ByteWidth = 64; //유의해야함 constant buffer에 넘기는 매개변수
    bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;//flag를 정해줘서 constnat 버퍼라는 것을 알려줌.

    dev->CreateBuffer(&bd, NULL, &pCBuffer);   // create the buffer

    devcon->VSSetConstantBuffers(0, 1, &pCBuffer); //vertec shader에게 constant buffer임을 알려줌.
}
